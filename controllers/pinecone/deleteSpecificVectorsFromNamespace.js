...
const deleteSpecificVectorsFromNamespace = async (req, res) => {
  try {
    const data = await Pinecone.deleteSpecificVectorsFromNamespace(
      req.body.indexName, req.params.id
    )
...
router.post(
  '/deleteSpecificVectorsFromNamespace/:id',
  validation,
  deleteSpecificVectorsFromNamespace
)
